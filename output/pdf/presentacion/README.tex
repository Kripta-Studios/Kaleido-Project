...
\section{Presentacion}\label{presentacion}
...
\tightlist
\item
  \texttt{Kaleido\_FlowTwin\_Presentacion.pdf}: version para proyectar o
  enviar.
\item
  \texttt{Kaleido\_FlowTwin\_Presentacion.html}: version offline;
  flechas para navegar, \texttt{O} para vista general, \texttt{N} para
  notas y \texttt{F} para pantalla completa.
\item
  \texttt{Kaleido\_FlowTwin\_Presentacion.tex}: fuente Beamer editable.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{latexmk }\OperatorTok{{-}}\NormalTok{pdf }\OperatorTok{{-}}\NormalTok{interaction}\OperatorTok{=}\NormalTok{nonstopmode }\OperatorTok{{-}}\NormalTok{halt{-}on{-}error Kaleido\_FlowTwin\_Presentacion}\OperatorTok{.}\FunctionTok{tex}
\end{Highlighting}
\end{Shaded}

Los auxiliares de LaTeX estan ignorados por el \texttt{.gitignore} del
proyecto.
